...
const KakaoMap = dynamic(() => import('@/components/map/KakaoMap'), {
  ssr: false,
  loading: () => <div className={styles.mapContainer}>지도를 불러오는 중...</div>,
});
...
export default function SpaceEditPage() {
    ...
    async function fetchSpace() {
      try {
        ...
        setForm({
          ...
          uploadDate: space.uploadDate ? new Date(space.uploadDate) : new Date(),
  ...
  if (loading || !form) return <div className={styles.container}>불러오는 중...</div>;
  ...
  return (
    <main className={styles.container}>
      ...
      <form onSubmit={handleSubmit} className={styles.form}>
        ...
        {/* 공간명 */}
        <div className={styles.section}>
          <label className={styles.label}>공간명 <span className={styles.required}>*</span></label>
          <input
            type="text"
            value={form.spaceName}
            onChange={e => setForm(f => f && { ...f, spaceName: e.target.value })}
        ...
        <div className={styles.section}>
          ...
          <input
            type="number"
            value={form.hostId ?? ''}
            onChange={e => setForm(f => f && { ...f, hostId: e.target.value === '' ? undefined : Number(e.target.value) })}
            className={styles.input}
        ...
        {/* 공간 소개 */}
        <div className={styles.section}>
          <label className={styles.label}>공간 소개 <span className={styles.required}>*</span></label>
          <textarea
            ...
            required
          />
          <p className={styles.charCount}>({form.description.length}/500자) 최소 20자 이상 입력해주세요.</p>
        </div>
        {/* 최대 수용 인원 */}
        <div className={styles.section}>
          <label className={styles.label}>최대 수용 인원 <span className={styles.required}>*</span></label>
          <div className="flex items-center">
            <input
              type="number"
              value={form.maxCapacity}
              onChange={e => setForm(f => f && { ...f, maxCapacity: Math.max(1, Math.min(1000, parseInt(e.target.value) || 1)) })}
          ...
          <div className="flex items-center">
            <input
              ...
              onChange={e => {
                ...
                setForm(f => f && { ...f, price: value === '' ? 0 : parseInt(value) });
              }}
              className={styles.numberInput}
              min="0"
              required
        ...
        <div className={styles.section}>
          ...
          <div className="space-y-4">
            ...
            <input
              type="text"
              value={form.address2}
              onChange={e => setForm(f => f && { ...f, address2: e.target.value })}
              className={styles.input}
            ...
            <div>
              <input
                type="text"
                value={form.address3}
                onChange={e => setForm(f => f && { ...f, address3: e.target.value })}
                className={styles.input}
        ...
        <div className={styles.footer}>
          <div className={styles.footerContent}>
            <button type="button" onClick={() => router.back()} className={styles.buttonSecondary}>이전</button>
            <button
              type="submit"
              className={
                !form.spaceName ||
